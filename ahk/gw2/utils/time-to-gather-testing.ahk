...
; each mode has 3 synonyms: full name, first letter of the name and a number - those all mean the same mode. The 4th item in the array, is the expected Return from the Noninterative use of the function
; the first array in the array of arrays is the default option
Map_Modes := Map()

Map_Modes["Rocks"] := Map("Synonyms", Array(), "ExpectedOutput", '')
Map_Modes["Lumber"] := Map("Synonyms", Array(), "ExpectedOutput", '')
Map_Modes["Bush"] := Map("Synonyms", Array(), "ExpectedOutput", '')

Map_Modes["Rocks"]["Synonyms"] := ["Rocks", "R", "1"]
Map_Modes["Rocks"]["ExpectedOutput"] := ["Rocks or 1 or R"]

Map_Modes["Lumber"]["Synonyms"] := ["Lumber", "L", "2"]
Map_Modes["Lumber"]["ExpectedOutput"] := ["Lumber or L or 2"]

Map_Modes["Bush"]["Synonyms"] := ["Bush", "L", "2"]
Map_Modes["Bush"]["ExpectedOutput"] := ["Bush or B or 3"]

for id, value IN Map_Modes["Rocks"]["Synonyms"]
MsgBox value
